...
export async function getTelegramMessagesByDate(dateStr: string): Promise<TelegramMessage[]> {
  ...
  return collection.find({
    create_at: dateStr
  }).sort({ create_at: -1 }).toArray() as Promise<TelegramMessage[]>;
}